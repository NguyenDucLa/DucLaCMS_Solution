        ...
        <div className="card shadow-sm border-0 rounded-lg">
            ...
            <div className="card-body p-0">
                <div className="list-group list-group-flush">
                    {/* Nút "Tất cả sản phẩm" luôn ở trên cùng */}
                    <button
                        type="button"
                        className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center px-4 py-3 transition-all ${selectedCategoryId === null ? 'active' : ''}`}
                        style={{ fontSize: '0.95rem' }}
                        onClick={() => onSelectCategory(null)}
                    >
                        <span className="font-weight-bold">
                            <i className="fa-solid fa-th-list mr-2"></i>Tất cả sản phẩm
                        </span>
                    ...
                    {categoryProducts.length === 0 ? (
                        ...
                        categoryProducts.map((item) => (
                            <button
                                key={item.id}
                                type="button"
                                className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center px-4 py-3 transition-all ${selectedCategoryId === item.id ? 'active' : ''}`}
